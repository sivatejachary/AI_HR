...
logger = logging.getLogger("zoom_provider")


class ZoomProvider(MeetingProvider):
    """
    Zoom implementation of MeetingProvider.
    Encapsulates Zoom Meeting REST API & Web SDK bot connection.
    """
    ...
    def join(self, meeting: Dict[str, Any]) -> Dict[str, Any]:
        if not self.validate_meeting(meeting):
            return {"status": "FAILED", "reason": "Invalid Zoom URL format", "meeting_status": "FAILED"}

        prep = self.prepare_meeting(meeting)
        m_id = meeting.get("id") or prep["meeting_id"]
        sess_id = f"zoom_sess_{int(time.time())}"
        
        self._active_sessions[m_id] = {
            "session_id": sess_id,
            "status": "CONNECTED",
            "ai_participant_id": f"part_zoom_ai_{sess_id[:6]}",
    ...
    def get_participants(self, meeting: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        return [
            {
                "participant_id": session.get("ai_participant_id", "part_ai_zoom"),
            ...
            {
                "participant_id": f"part_cand_zoom_{m_id}",
    ...
    def get_active_speaker(self, meeting: Dict[str, Any]) -> Optional[str]:
        return f"part_cand_zoom_{meeting.get('id')}"
